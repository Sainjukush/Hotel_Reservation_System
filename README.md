...
3. Execute in order:
   ...
   - Lookup seeding
   - Stored procedures
   - Test execution blocks (optional)

> Recommendation: Run the script in sections if you are actively developing to avoid re-running inserts unintentionally.

---

## Suggested improvements (optional)

- Fix the ID mapping in `SpPersonIns` and `SpAddressIns` to reliably pair input JSON rows with inserted IDs for multi-row inserts.
- Split “schema”, “seed”, “procedures”, and “tests” into separate SQL files for cleaner execution.
- Align demo JSON queries with the actual schema (or add the missing tables/columns if they are part of the intended design).
